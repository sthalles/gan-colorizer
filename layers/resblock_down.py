...
class BlockDown(tf.keras.Model):
    def __init__(self, in_channels, out_channels, hidden_channels=None,
        ...
        self.c1 = SNConv2D(hidden_channels, kernel_size=ksize, padding=pad, kernel_initializer=initializer, use_bias=False)

        self.b1 = tf.keras.layers.BatchNormalization()
        self.b2 = tf.keras.layers.BatchNormalization()

        self.c2 = SNConv2D(out_channels, kernel_size=ksize, padding=pad, kernel_initializer=initializer, use_bias=False)
        self.avg_pool = tf.keras.layers.AveragePooling2D(pool_size=2, strides=2, padding="SAME")

        if self.learnable_sc:
            self.c_sc = SNConv2D(out_channels, kernel_size=1, padding="VALID", kernel_initializer=initializer, use_bias=False)
