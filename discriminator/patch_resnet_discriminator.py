...
from layers.resblock_down import BlockDown, OptimizedBlock
...
class SNResNetPatchGanDiscriminator(tf.keras.Model):
    def __init__(self, ch=64, activation=tf.keras.layers.ReLU()):
        ...
        self.block2 = BlockDown(ch * 2, ch * 4, ksize=4, activation=activation, downsample=True)
        self.block3 = BlockDown(ch * 4, ch * 8, ksize=4, activation=activation, downsample=True)
    ...
    def __call__(self, x, sn_update, **kwargs):
        ...
        h = self.block2(h, sn_update=sn_update, **kwargs) # (bs, 64, 64, DEPTH)
        h = self.block3(h, sn_update=sn_update, **kwargs) # (bs, 32, 32, DEPTH)
