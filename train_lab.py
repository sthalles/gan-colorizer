...
from discriminator.snresnet_128 import SNResNetProjectionDiscriminator
...
test_dataset = tfds.load(name=DATASET_NAME, split=tfds.Split.ALL)
...
def generate_images(model, L_batch, AB_batch):
  ...
  fake_image = model(L_batch, sn_update=True, training=True)
...
def generator_train_step(L_batch, AB_batch):
  with tf.GradientTape() as gen_tape:
    ...
    disc_fake_loss = discriminator(tf.concat([L_batch, fake_image], axis=3), sn_update=False)
    ...
    # l1_loss = gen_l1_loss(fake_image, AB_batch, lambda_=10)
    gen_loss = hinge_loss + regularization_loss
